   ...
   public void tickMovement() {
      ...
      if (this.world.isClient) {
         if (this.random.nextInt(24) == 0 && !this.isSilent()) {
            this.world.playSound(this.getX() + 0.5D, this.getY() + 0.5D, this.getZ() + 0.5D, glomod.NOTHINGEVENT,
                  this.getSoundCategory(), 1.0F + this.random.nextFloat(), this.random.nextFloat() * 0.7F + 0.3F,
                  false);
         }

         for (int int_1 = 0; int_1 < 2; ++int_1) {
            this.world.addParticle(ParticleTypes.ITEM_SNOWBALL, this.getParticleX(0.5D), this.getRandomBodyY(),
